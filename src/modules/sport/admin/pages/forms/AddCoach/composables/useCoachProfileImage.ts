...
import { ALLOWED_PROFILE_IMAGE_TYPES, IMAGE_OPTIMIZATION_OPTIONS, MAX_PROFILE_IMAGE_SIZE_BYTES } from '../constants/addCoachConstants'

export function useCoachProfileImage(t: any) {
  ...
  async function handleProfileImageChange(event: Event, form: any, isFormLocked: boolean): Promise<string> {
    ...
    if (!ALLOWED_PROFILE_IMAGE_TYPES.includes(file.type)) {
      return t('sportAddCoach.validation.imageType')
    }

    if (file.size > MAX_PROFILE_IMAGE_SIZE_BYTES) {
      return t('sportAddCoach.validation.imageSize')
    }

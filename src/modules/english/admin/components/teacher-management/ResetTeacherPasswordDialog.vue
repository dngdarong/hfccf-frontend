...
function validateForm() {
  ...
  if (!normalizedPassword) {
    setValidationError('password', t('english.teachers.validation.passwordRequired'))
    isValid = false
  } else if (normalizedPassword.length < MIN_PASSWORD_LENGTH) {
    setValidationError(
      'password',
      t('english.teachers.validation.passwordTooShort', { count: MIN_PASSWORD_LENGTH }),
    )
    isValid = false
  }

  if (!normalizedConfirm) {
    setValidationError('confirmPassword', t('english.teachers.validation.confirmPasswordRequired'))
    isValid = false
  } else if (normalizedPassword !== normalizedConfirm) {
    setValidationError('confirmPassword', t('english.teachers.validation.passwordMismatch'))
    isValid = false
  }

  if (!normalizedReason) {
    setValidationError('reason', t('english.teachers.validation.reasonRequired'))
    isValid = false
  }
...
<template>
  <Dialog
    :visible="visible"
    modal
    :header="t('english.teachers.resetPasswordTitle')"
  ...
  >
    <div class="reset-teacher-password-dialog__content">
      <p class="reset-teacher-password-dialog__description">
        {{ t('english.teachers.resetPasswordDescription') }}
      </p>
      ...
      <label class="reset-teacher-password-dialog__field">
        <span>{{ t('english.teachers.newPassword') }}</span>
        <input
          v-model="password"
          type="password"
          autocomplete="new-password"
          class="reset-teacher-password-dialog__input"
          :placeholder="t('english.teachers.newPassword')"
        >
      ...
      <label class="reset-teacher-password-dialog__field">
        <span>{{ t('english.teachers.confirmPassword') }}</span>
        <input
          v-model="confirmPassword"
          type="password"
          autocomplete="new-password"
          class="reset-teacher-password-dialog__input"
          :placeholder="t('english.teachers.confirmPassword')"
        >
      ...
      <label class="reset-teacher-password-dialog__field">
        <span>{{ t('english.teachers.reason') }}</span>
        <textarea
          v-model="reason"
          rows="4"
          class="reset-teacher-password-dialog__textarea"
          :placeholder="t('english.teachers.reasonPlaceholder')"
        />
    ...
    <template #footer>
      ...
      <Button
        ...
        rounded="xl"
        :loading="loading"
        :label="t('english.teachers.resetPassword')"
        @click="submit"
      />
    </template>
  </Dialog>
